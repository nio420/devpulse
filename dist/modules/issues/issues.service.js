import { pool } from "../../db";
const createIssueDB = async (payload, reporter_id) => {
...
const getAllIssuesDB = async (query) => {
...
const getSingleIssueDB = async (id) => {
    const result = await pool.query(`
...
const updateIssueDB = async (id, payload, user) => {
    const issueResult = await pool.query(`SELECT * FROM issues WHERE id = $1`, [id]);
    if (issueResult.rows.length === 0) {
        throw new Error("Issue not found.");
    }
    const issue = issueResult.rows[0];
    const userResult = await pool.query(`SELECT id, role FROM users WHERE id = $1`, [user.id]);
